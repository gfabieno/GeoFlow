...
class GraphOutput:
    """
    A class to generate the output, label and weight of an network.
    """
    name = "Baseoutput"

    def __init__(self, model: BaseModelGenerator, acquire: Acquisition):
        """
        To generate the output, we need to know what is the input data, which
        is performed by model and acquire. Additional attributes that control
        how an output is generated can be defined here.
    ...
    def plot(self, data, ax=None, cmap=plt.get_cmap('hot'), vmin=0,
        ...
        :param vmax: Maximum value of the colormap. If None, default to
                     clip * max of data
        :param clip: Cliping of the data
    ...
    def generate(self, props):
        """
        From a dict of properties, the labels are generated by this function.
        The labels can be saved in a format and later transformed by preprocess
        if the training requires a modification of the label.

        :param props: A dict of {propname: np.array) containing the earth
                      properties
        :returns label: An np.array containing the label
                 weight: An np.array containing the weight. When computing the
    ...
    def preprocess(self, label, weight):
        """
            A function to preprocess the label before feeding it to the network.

            @params:
            label: An np.array containing the label
            weight: An np.array containing the weight. When computing the
                         loss, the outputs and labels are multiplied by weight.

            @returns:
            data (np.array): The preprocessed data ready to be fed to the
                             network
        """
        raise NotImplementedError
    ...
    def postprocess(self, label):
        """
        A function to postprocess the outputs.

        @params:
        label  (np.array): The output to postprocess

        @returns:
        labels (dict):      The preprocessed output
        """
        raise NotImplementedError
...
class Vdepth(Vrms):
    ...
    def generate(self, props):
        ...
        for ii in range(vp.shape[1]):
            i_t = np.argwhere(refs[:, ii] > 0.1).flatten()[-1]
            mask = dweights[:,
                   ii] >= i_t * self.acquire.dt * self.acquire.resampling
            dweights[mask, ii] = 0
...
class GraphInput:
    ...
    def plot(self, data, ax, cmap=plt.get_cmap('Greys'), vmin=None, vmax=None,
             clip=0.1, im=None):
        """
        Default plotting of this input
    ...
    def generate(self, data):
        """
           A function to compute the GraphInput from the modelled data to be
           saved on disk.
       """
        return data
    ...
    def preprocess(self, data, labels):
        """
           A function to preprocess the data before feeding it to the network.

           @params:
           data (np.array): The input data
           labels  (dict): A dict of labels {name: np.array}
...
class ShotGather(GraphInput):
    ...
    def __init__(self,
        ...
        """
        Parameters controling the preprocessing:

        :param acquire: An acquisition object controlling data creation
        :param model: A BaseModelGenerator
